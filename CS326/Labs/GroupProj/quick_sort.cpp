...
int partitionLA(int arr[], int start, int end)  // Lamuto
{
  ...
  int j = start -
          1;  // index of smalllest element in array - 1 as j starts next to it

  for (int i = start; i <= end - 1; i++)  // go thru array
  {
    if (arr[i] <= pivot)  // if the element if smaller than swap
    {
      j++;  // move i over first so you dont constantly swap in the same
            // position
      swap(arr[j], arr[i]);
    }
  }
  swap(arr[j + 1],
       arr[end]);  // move the pivot into the array from the space next to i
  return (j + 1);  // place of pivot in the array
}

void QuicksortLA(int arr[], int start, int end) {
  if (start <
      end)  // as long as the quicksort ran thru all elements in the array
  {
    int partIndex =
        partitionLA(arr, start, end);  // sort array and get partition index
    QuicksortLA(arr, start,
                partIndex - 1);  // redo recursevely for each subarray
...
void QuicksortLB(int arr[], int start, int end) {
  ...
  {
    int partIndex =
        partitionLB(arr, start, end);  // sort array and get partition index
    QuicksortLB(arr, start,
                partIndex - 1);  // redo recursevely for each subarray
...
int partitionHAS(int arr[], int start, int end) {  // Hoarse
  int pivot = arr[start];
