...
void mergeSortA(int arr[], int start, int end) {  // must do size - 1
  if (start >=
...
void mergeSortB(int arr[], int start, int end) {  // must do size - 1
  if (start >=
      end)  // base case, if start is >= end we have proccessed all the indexes
  {
    return;
...
void mergeInsertionSortA(
    int arr[], int start, int stop,
    int k)  // k is switch from merge to insertion 5% of the total array size
{
  if (stop - start <= k) {
    ...
    mergeInsertionSortA(arr, start, mid,
                        k);  // partition 1 half of current full array
    mergeInsertionSortA(arr, mid + 1, stop,
...
void mergeInsertionSortB(int arr[], int start, int stop, int k) {
  if (stop - start <= k) {
    ...
    int mid = start + (stop - start) /
                          2;  // midpoint to partiton is half of array length
    mergeInsertionSortB(arr, start, mid,
                        k);  // partition 1 half of current full array
    mergeInsertionSortB(arr, mid + 1, stop,
                        k);  // partition 2 half of current full array
