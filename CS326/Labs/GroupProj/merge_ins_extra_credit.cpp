...
void func_evalMergeIns(
    int arr[], int n, int m, double k,
    void (*func)(int[], int, int,
                 double))  // for merge ins sort, has extra parameter
{
...
void mergeInsertionSort(
    int arr[], int start, int stop,
    double k)  // k is switch from merge to insertion 5% of the total array size
{
  if (stop - start <= k) {
    ...
    int mid = start + (stop - start) /
                          2;  // midpoint to partiton is half of array length
    mergeInsertionSort(arr, start, mid,
                       k);  // partition 1 half of current full array
    mergeInsertionSort(arr, mid + 1, stop,
                       k);  // partition 2 half of current full array
...

{
  ...
  for (int i = 0; i < 1; i++)  // for all datasetsizes
  {
    ...
    cout << setw(15) << left << "Portion of N" << setw(15) << "Portion Value"
         << setw(15) << "CPU AVG" << right << endl;
    for (int i = 1; i <= 50; i++)  // for 1 - 50 percent of dataset size
    {
      ...
      for (int j = 1; j <= 50;
           j++)  // evaluate avg of each percent for that datasetsize
      {
        int* tempArr = newPermutArray(Size);

        func_evalMergeIns(tempArr, 0, Size - 1, Size * SizePercent,
                          mergeInsertionSort);

        delete[] tempArr;
        BaseArr.clear();
      }
      cout << setw(15) << left << SizePercent << setw(15) << Size * SizePercent
           << setw(15) << CPUTotal / 50 << right << endl;
    }
  }
  ...
  //     cout << setw(15) << left << "Portion of N" << setw(15) << "Portion
  //     Value" << setw(15) << "STEP AVG" << right << endl; for (int i = 1; i <=
  //     50; i++) // for 1 - 50 percent of dataset size
  //     {
  //         SizePercent += 0.01;
  //         StepTotal = 0;

  //         for (int j = 1; j <= 50; j++) // evaluate avg of each percent for
  //         that datasetsize
  //         {
  //             int *tempArr = newPermutArray(Size);

  //             func_evalMergeInsSteps(tempArr, 0, Size - 1, Size *
  //             SizePercent, mergeInsertionSortS);

  //             delete[] tempArr;
  //             BaseArr.clear();
  //         }
  //         cout << setw(15) << left << SizePercent << setw(15) << Size *
  //         SizePercent << setw(15) << StepTotal / 50 << right << endl;
  //     }
  // }
