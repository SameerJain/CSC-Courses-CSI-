...
class arrayStack {
 public:
  ...
  void push3(
      double x) {  // for operand operation. leaves int value and doesnt convert
    if (isFull()) {
      cout << "Stack is Full\n";
    } else {
      top++;
  ...
  {
    if (entry == '+' ||
        entry == '-')  // lower priority for addition and subtraction
    {
      return (1);
    }

    else if (entry == '*' ||
             entry == '/')  // higher priority for multiplication and division
    {
      return (2);
    }
...
arrayStack
    s;  // declaration for array based stack to be used in main for simplicity
...
struct pStack* createstack(
    unsigned capacity)  // creats new stack, capacity must be positive
{
  // defines size
  ...
  for (int i = 0; i < stack->capacity;
       i++) {  // sets entire stack equal to zero for less memory usage
    stack->array[i] = 0;
  }
...
bool isEmpty(struct pStack* stack) {
  if (stack->top == -1) {  // if top is less than index for first element
    return true;
  } else {
    return false;
  }
...
char peek(
    struct pStack* stack)  // gets the first element of stack without dequeuing
{
...
void push(struct pStack* stack, int op)  // 1 digit values
{
  stack->array[++stack->top] =
      op;  // sets the top pinter up one then assigns it the pushed value
}

void push2dig(struct pStack* stack, string op)  //  2 digit values
{
  double x =
      stoi(op);  // converts string to double to then be pushed onto stack
...
string convert_postfix(string infix) {
  ...
  while (infix[i] != '\0')  // for the whole loop
  {
    ...
    else if (infix[i] == ')')  // if (
    {
      while (x.get_top() != '(' && !x.isEmpty()) {
        postfix.insert(
            postfix.end(),
            x.pop());  // Enter in everything on stack for whats in ( )
      }
      i++;
...

    {
      if (x.isEmpty()) {
        ...
        if (x.priority(infix[i] > x.priority(x.get_top()))) {
          x.push(infix[i]);  // if priotity is higher than or equal to top of
                             // stack then push it in front
          ...
          while ((!x.isEmpty()) &&
                 x.priority(infix[i]) <= x.priority(x.get_top())) {
            postfix.insert(postfix.end(),
                           x.pop());  // if priotity is lower then push whatever
                                      // is on the stack until its empty
          }
  ...
  while (!x.isEmpty()) {
    postfix.insert(postfix.end(),
                   x.pop());  // pop any remaining expressions form stack
  }
