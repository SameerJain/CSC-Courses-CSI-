...
def check_if_sum_possible(arr, k):
    ...
    def _check_sum_possible(curr_index, sum_so_far, at_least_one):
        if curr_index >= length:
            ...
            # Some of those numbers we included in our group, others we left
out.
        ...
        # We explore both cases: either we include arr[curr_index] in the group
or we don't. # Returning True if either choice finds a group that sums up to
exactly k.
        return  _check_sum_possible(curr_index + 1, sum_so_far +
arr[curr_index], True) or \ _check_sum_possible(curr_index + 1, sum_so_far,
at_least_one)
...
int main() {
  ...
  // Array 2
  int arr2[20]{1,  5,  7,  34, 76, 54, 23, 19, 22, 81,
               44, 77, 29, 40, 11, 42, 43, 31, 57, 61};
...
bool partition(int arr[], int n,
               int target)  // Checks if there exists a subset of intergers that
                            // equal half the sum
{
  if (target == 0) {  // if target has reached 0, then a subset was found
    return true;
  }

  else if (n == 0) {  // or there wasnt a subset in which this was the case
    seqBank.clear();
    return false;
  }

  // if the next element is greater than the amount left, you know that you need
  // to move on
    ...
    return partition(arr, n - 1, target - arr[n - 1]) ||
           partition(arr, n - 1,
                     target);  // run thru array, and also check if it can be
                               // achieved from that same element
  }
}
