...
void insertList(int data) {
  Node* temp = new Node();  // temp node to iterate and new node to be passed
                            // into the end
  temp = head;
  ...
  if (head == NULL)  // if list is empty make head the new node
  {
    ...
    while (temp->next !=
           NULL)  // iterate to the end and make the new node the last node
    {
      temp = temp->next;
    }
    temp->next = newNode;
...
void printList() {
  ...
  while (temp != NULL) {  // iterate to the end and print null after last node
    cout << temp->data << "->";
    temp = temp->next;
  }
  cout << "NULL" << endl;
...
void deleteLastDup(int val) {
  ...
  else if (head->next == NULL)  // case if theres only one element in the list
  {
    ...
    while (temp->next != NULL) {      // iterate through the list
      if (temp->next->data == val) {  // if the next node is a duplicate
        lastDupPrev =
            temp;  // make this node the node before the last duplicate

        if (lastDup == NULL) {
          lastDup = temp->next;  // if this is the first duplicate then make it
                                 // the next one
        }

        else {
          prevDup =
              lastDup;  // if its not the first duplicate to appear, then make
                        // the last final duplicate the second to last,
          lastDup =
              temp->next;  // make the next one equal to the last duplicate
        }
      }
  ...
  {
    lastDupPrev->next =
        lastDup->next;  // make the node before the last duplicate point to the
                        // one after, as we are deleting
...
void deleteSecondLastDup(int val) {
  ...
  while (temp->next != NULL) {
    if (temp->next->data == val) {
      ...
      else {  // if there is more than one duplicate the previous lastdup is now
              // the second to last, and new one is the new last
        prevDup = lastDup;
        lastDup = temp->next;
      }
    }
  ...
  if (prevDup == head)  // if the second to last dup is head then delete and
                        // move over the head
  {
    head = head->next;
    delete prevDup;
    ...
    // if the second to last dup is not the head, iterate until you fing the
    // node before it and then move the arrow, delete it
    prevDupPrev = head;
  ...
  else if (prevDup == NULL)  // if theres only 1 instance of the duplicate
                             // number then its not a dup
  {
    cout << val << " is not Duplicate\n";
  }
}
