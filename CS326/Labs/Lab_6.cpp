...
Customer
Queue::getHead()  // returns value of start of queue without removing it
{
  if (tail == -1) {
    cout << "Queue is empty" << endl;
    Customer x;
...
void Queue::displayQueue() {
  ...
  if (isEmpty()) {
    ...
    for (int i = head; i <= tail; i++)  // uses setw for cleaner output
    {
      ...
      cout << setw(16) << left << "Customer Name: " << list[i].CustomerName
           << endl;
      cout << setw(16) << left << "Arrival Time: " << list[i].ArrivalTime
           << endl;
      cout << setw(16) << left << "Service Time: " << list[i].ServiceTime
           << endl;
      cout << setw(16) << left << "Finish Time: " << list[i].FinishTime << right
           << endl;
    }
    cout << endl;
  }
...
class CircularQueue {
 ...
 public:
  CircularQueue() {  // constructor that intializes indexs for front and back
  ...
  bool isFull() {
    if (front == 0 &&
        rear == SIZE - 1) {  // if the array has a full amount of elements in
                             // each index from 0 to the end
      return true;
    }

    if (front == rear + 1) {
      return true;  // circ queue has all elements fulled when rear flips back
                    // around
    }
  ...
  void enQueue(Customer data) {
    if (isFull())  // case if queue is full
    {
      ...
      count++;
      if (isEmpty()) {  // if its empty, move front node to arr[0]. dont do this
                        // if its not empty since we will lose an element
        front++;
      }
      rear =
          (rear + 1) % SIZE;  // rear is the remainder of its value div by size.
                              // ele 26 div by 25 is rem 1, so back to 1
  ...
  Customer Dequeue() {
    ...
    if (isEmpty()) {
      ...
      if (front == rear) {  // make the queue empty if there is only one element
        front = -1;
        rear = -1;
  ...
  void displayQueue() {
    ...
    if (isEmpty()) {
      ...
      while (c > 0) {
        ...
        cout << setw(16) << left << "Service Time: " << list[i].ServiceTime
             << endl;
        cout << setw(16) << left << "Finish Time: " << list[i].FinishTime
             << right << endl
             << endl;
        c--;
        i++;
...
void inputNodeData(Node* input, string name, int arrivalTime, int serviceTime,
                   int finishTime)  // function to setup node values for a
...
class CircPointerQueue {
 public:
  struct Node* head =
      NULL;  // start construction of LL queue with head and tail pointers
...
void CircPointerQueue::Enqueue(Node* cus) {
  ...
  tail->next =
      head;  // make tail next equal to head to keep Linked list circular
}
...
Node* CircPointerQueue::Dequeue() {
  ...
  if (head == NULL) {
    ...
    if (head == tail)  // if there is only 1 element in the array
    {
      ...
      tail->next =
          head;  // keep list circular and depoints the dequeue node out of list
...
class Stack {
 public:
  ...
  char pop() {
    ...
    {
      head--;
      return arr[head];  // when value is pushed, the previously poped value is
                         // re-written
    }
  }
};
...
bool isPalindrome(string input) {
  ...
  for (int i = 0; i < input.size();
       i++)  // loop enters string to both stack and queue
  {
  ...
  for (int i = 0; i < input.size();
       i++)  // loop dequeues all values from stack and queue
  {
    // if there is a difference, you know its not a palindrome
    if (q4Stack.pop() !=
        q4Queue.dequeue())  // this works because stacks and queues exit values
...
main() {
  ...
  cout << "\n           Q3 CIRCULAR POINTER BASED QUEUE\n\n" << endl;
  // create new Node struct for Tom and Bob to fit inside of circular array
  // based queue
  ...
  string inputWord;
  cout << "\n\n\n         Q4: PALINDROME CHECKER USING STACK AND "
          "QUEUE\n\n\nEnter a word:";
  cin >> inputWord;
