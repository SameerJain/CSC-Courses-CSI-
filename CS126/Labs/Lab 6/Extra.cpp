...
int main() {
  ...
  while (doneday == false) {  // stops when no customer left
    ...
    if (userEnt == "cash") {  // puts money into reg total
      regsum += billsum;
    }
      ...
      if (userEnt == "Y") {  // money taken out of register
        cout << "Enter Amount:\n";
        cin >> cashback;
        regsum -= cashback;
      }

      else if (userEnt == "N") {  // No change to total
        cout << "No cashback!\n";
      }
    }

    if (regsum >= 0) {  // special case if cash in register is less than zero
    ...
    if (moreCus == 'Y') {  // loops back
      doneday = false;
    }

    if (moreCus == 'N') {  // breaks out
      doneday = true;
    }
  }
