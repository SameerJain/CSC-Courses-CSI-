...
d. trail = current->link->link; (from current yellow box move 2 yellow boxes
down) trail->link = nullptr; cout << trail->info; (blue box at the end of
linking) 46 e. cout << last->link->info;
...
last = trail;
Answer of 3. a
Sets the link field of the node with info 5 to nullptr, deletes the last node
from the linked list, and deallocates the memory occupied by the last note, and
after deleting the last node sets list point to the last node of the list. The
last node is seen to be completely deleted and replaced as trail as the last
mode. The linked list goes from 7 to 8 nodes. b. temp->link = last; This would
mean that the pointer to show how its the last node in the linked list. c.
first->info = 58; replaces 47 with 58 d. q = current->link; current->link =
temp; delete q;
...
5. Mark each of the following statements as valid or invalid. If a statement is
invalid, explain why. a. p = list->link; b. first = list; c. temp->link =
nullptr; d. current->link = temp->info; e. p = *last; f. first = 90; g.
p->link->info = current->info; h. current->info = temp->link; i. *list = *temp;
...
c. Delete the last node of the list and also deallocate the memory occupied by
this node. After deleting the node make last point to the last node of the list
and the link of the last node must be nullptr. d. Delete the node with info 92.
Also, deallocate the memory occupied by this node. e. Write a C++ code to move
the node with info 46 after current by adjusting the links of nodes in the
linked list.
...
8. If the following C++ code is valid, show the output. If it is invalid,
explain why. q = current; current = current->link; current->link = temp->link;
trail = current->link;
current = trail->link;
cout << q->info << " " << trail->info << " " << current->info << endl;

9. If the following C++ code is valid, show the output. If it is invalid,
explain why. trail = temp->link; //Line 1 temp = temp->link->link; //Line 2
current = temp; //Line 3
